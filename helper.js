  ...
  for (let pd in pack.peerDependencies) {
    if (pd === 'eslint') {
      ...
      for (const ruleId in typescript_eslint_rules) {
        if (
          !typescript_eslint_rules[ruleId].meta ||
          !typescript_eslint_rules[ruleId].meta.deprecated
        ) {
...
function getConfig() {
  ...
  const baseRulesCurrentValues = currentRules.find((value) =>
    arrayEqual(value.files, ['./credentials/**/*.ts', './nodes/**/*.ts']),
  );
  const nodesRulesCurrentValues = currentRules.find((value) =>
    arrayEqual(value.files, ['./nodes/**/*.ts']),
  );
  const credentialsRulesCurrentValues = currentRules.find((value) =>
    arrayEqual(value.files, ['./credentials/**/*.ts']),
  );
  const jsonRulesCurrentValues = currentRules.find((value) =>
    arrayEqual(value.files, ['package.json']),
  );
  ...
  function getCurrentValue(currentValues, rulename, rule) {
    return {
      value:
        currentValues && currentValues.rules && currentValues.rules[rulename]
          ? currentValues.rules[rulename]
          : rule && rule.meta && rule.meta.docs
    ...
    if (plugin == 'eslint-plugin-n8n-nodes-base') {
      for (const rule in rules[plugin]) {
        if (rule.startsWith('n8n-nodes-base/community-package-json-')) {
          ...
          credentialsRules[rule] = getCurrentValue(
            credentialsRulesCurrentValues,
            rule,
            rules[plugin][rule],
          );
    ...
    default:
      ...
      return types.objectExpression(
        Object.keys(value).map((v) =>
          types.objectProperty(types.stringLiteral(v), buildAst(value[v])),
        ),
      );
  }
}
...
function getConfigAst() {
  ...
  configAst.properties.push(
    types.objectProperty(
      types.stringLiteral('overrides'),
      types.arrayExpression(
        overrides.map((value) =>
          types.objectExpression([
            ...
            types.objectProperty(
              types.stringLiteral('rules'),
              types.objectExpression(
                Object.keys(value.rules).map((rule) =>
                  value.rules[rule].description
                    ? types.addComment(
                        types.objectProperty(
                          types.stringLiteral(rule),
                          buildAst(value.rules[rule].value),
                        ),
                        'trailing',
                        value.rules[rule].description,
                        true,
